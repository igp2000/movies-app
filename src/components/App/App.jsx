...
export default class App extends Component {
  ...
  getGenres() {
    this.SearchMovies.getGenres().then((list) => {
      if (list.genres) {
      ...
      .then((list) => {
        ...
        const newList = list.results.map((item) => {
          ...
          return {
            ...
            vote_average: ind > -1 ? ratedMovies.listMovies[ind].vote_average : 0,
          };
        });
  ...
  onChangeRate = (id, number) => {
    ...
    if (ind > -1) {
      this.onlineData.listMovies[ind].vote_average = number;
      if (!this.state.tabRated) {
    ...
    if (indRated > -1) {
      ratedMovies.listMovies[indRated].vote_average = number;
      if (this.state.tabRated) {
